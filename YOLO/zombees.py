...
crop_x1, crop_y1 = 120, 12	0  # Example coordinates (adjust as needed)
...
# Turbine countdowners init
turbine_frames = 5 #5 frames to make decision
turbine_Start_frame =0
turbine_counter=0
frame_number =0
turbine_on_time = None
turbine_off_time = None
# function checking is it the correct frame to check
...
#in-cycle check that turbine is curently not cleaning the tunnel.
# I'm sorry this should be here :)
#############
  if turbine_on_time is not None and time.time()*1000 > turbine_on_time + turbine_working_interval_ms:	  		
      osd_status ="TURBINE OFF"
      control_gpio(initialized_lines, CH1_LINE,"Channel 1", "OFF")    	
      print (osd_status)
      turbine_off_time = time.time()*1000 
      turbine_on_time = None
      cv2.imwrite("back.jpg", background_frame) # updating background picture
############
...
# Check if motion is detected and display appropriate text
  if changed_cells.any():
       ...
       #for box, track_id, conf, cls in zip(boxes, track_ids, confs, clss):
          for box, conf, cls in zip(boxes, confs, clss):
              ...
              if int(cls) == 0:  # larry
                  ...
                  osd_status ="ALERT"
                  print (osd_status)
                  if turbine_Start_frame ==0:
                      turbine_Start_frame = frame_number
                  print (turbine_on_time,   time.time()*1000)
                              
                  if turbine_counter == turbine_frames and frame_number == turbine_Start_frame+turbine_frames-1:   # OMG! There is LARRY coming to beehive!!!
                      if turbine_on_time is None: 
                          osd_status ="TURBINE ON"
                          control_gpio(initialized_lines, CH1_LINE,"Channel 1", "ON")  # get rid of Larry  	
                          print (osd_status)
                          turbine_on_time = time.time()*1000
                          print (turbine_on_time)
                           
                      
                      
                  if turbine_counter != turbine_frames and frame_number	>= 	turbine_Start_frame+turbine_frames:  #object was detected as larry with interruptions
                      turbine_Start_frame =0
                      turbine_counter=0 
	  ...
	  print (osd_status)
  if turbine_on_time is not None:
	  osd_status = ("TURBINE ON")
